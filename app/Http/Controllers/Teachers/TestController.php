...
class TestController extends Controller
{
    ...
    {
        $tests = Test::where('teacher_id', Auth()->user()->id)->latest()->get()
    ...
    public function results()
    {
        ...
        $tests = Test::where('teacher_id', Auth::user()->id)
            ->whereIn('id', $completed)->latest()->get()->transform(function (
                $test
            ) {
                $result             = new stdClass;
                $result->id         = $test->id;
                ...
                $student_test            = StudentTest::where('test_id',
                    $test->id)
                    ->first();
                $result->student_test_id = $student_test->id;
                $result->student_name    = $student_test->student->name;
                $result->created_at
                                         = $student_test->created_at->diffForHumans();
                $result->marks_obtained  = 0;
                foreach ($student_test->questions as $q) {
                    $result->marks_obtained += $q->marks();
                }
    ...
    public function view(Request $request, $id)
    {
        ...
        $test->questions = $student_test->questions->transform(function (
            $question
        ) {
            $result                 = new stdClass;
    ...
    public function store(Request $request)
    {
        $data = $this->validate($request, [
            ...
            'left_operand.*'  => ['required', 'numeric'],
            'right_operand.*' => ['required', 'numeric'],
        ...
        try {
            ...
            $questions = $this->prepareQuestions($test->id,
                array_map(
                    function ($lo, $o, $ro) { return "$lo $o $ro"; },
                    $data['left_operand'],
                    $data['operation'],
                    $data['right_operand']
                ), array_map(
                    function ($lo, $o, $ro) {
                        return ($o === '*') ? ($lo * $ro) : ($lo / $ro);
                    },
                    $data['left_operand'],
                    $data['operation'],
                    $data['right_operand']
